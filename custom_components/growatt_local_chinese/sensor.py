...
class GrowattDeviceEntity(CoordinatorEntity, RestoreEntity, SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        # 只对状态传感器进行特殊处理
        if self.entity_description.key == "status" and self.hass.config.language == "zh-Hans":
            # 状态翻译字典
            status_translations = {
                "Offline": "离线",
                "not_connected": "未连接",
                ...
                "PV_charge": "光伏充电",
                "AC_charge": "交流充电",
                "Combined_charge": "组合充电",
                "Combined_charge_bypass": "组合充电旁路",
                "PV_charge_bypass": "光伏充电旁路",
                "AC_charge_bypass": "交流充电旁路",
                "Bypass": "旁路",
                "PV_charge_discharge": "光伏充放电",
                "Not_used": "未使用",
                "Stendby": "待机"
            }
            # 如果状态在翻译字典中，则使用翻译后的值
            if state in status_translations:
                self._attr_native_value = status_translations[state]
            else:
